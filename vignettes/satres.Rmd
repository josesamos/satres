...
By default, only the satellite spectral bands (*B01* to *B12*) are considered. By setting the `only_spectral` parameter to `FALSE`, we can indicate that all raster files included in the folders that cover the area of interest are also treated, as shown in the code below.

```{r, eval=FALSE}
sr_all <- satres(dir = "esa", out_dir = "esa/out", only_spectral = FALSE)
```
